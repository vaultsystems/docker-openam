...
docker run -v $PWD:/mnt ubuntu:wily bash -c "apt-get update && apt-get install -y default-jdk maven && tar xzf /mnt/openam-source.tar.gz && cd openam && MAVEN_OPTS=-Xmx1024m mvn -T 1C -DskipTests=true clean install && cp openam-server/target/*.war /mnt/openam.war"
